<div class="flex h-full" x-data="{
    currentEntry: $wire.entangle('currentEntry'),
}">
    ...
    <div class="hidden lg:flex lg:flex-shrink-0">
        <div class="flex flex-col w-64">
            ...
            <div class="flex flex-col flex-1 min-h-0 border-r border-gray-200 dark:border-black">
                <div class="flex flex-col flex-1 pt-4 pb-4 overflow-y-auto">
                    @if ($feeds)
                        <nav class="flex-1" aria-label="Feeds">
                            <div class="px-2 space-y-1">
                                @foreach ($feeds as $feed)
                                    <!-- Current: "bg-gray-200 text-gray-900", Default: "text-gray-600 hover:bg-gray-50 hover:text-gray-900" -->
                                    <button type="button" wire:click.prevent="setCurrentFeed({{ $feed->id }})"
                                        @class([
                                            'flex items-baseline justify-between w-full px-2 py-2 text-sm font-medium rounded-md hover:text-gray-800 hover:bg-gray-100 hover:dark:text-gray-100 hover:dark:bg-gray-800',
                                        ...
                                        @if ($counts[$feed->id] > 0)
                                            <span class="text-xs text-orange-500">{{ $counts[$feed->id] }}</span>
                                        @endif
                                    </button>
                                @endforeach
                            </div>
        ...
        <div class="relative z-0 flex flex-1 overflow-hidden">
                ...
                <div class="absolute inset-0 px-4 py-6 sm:px-6 lg:px-8">
                    @if ($currentEntry)
                        <article class="mx-auto prose lg:prose-lg dark:prose-invert">
                            <h1 class="lg:leading-tight">{{ $currentEntry->title }}</h1>
                            {!! Purify::clean($currentEntry->summary) !!}
                            <p>
                                <a href="{{ $currentEntry->link }}"
                                    class="inline-flex items-center gap-3 px-5 py-3 text-lg text-white no-underline bg-blue-500 rounded hover:bg-blue-600 focus:bg-blue-600"
                                    target="_blank">
                                    <span>Read this article</span>
                                    <span class="sr-only">(Opens in a new window</span>
                                    <x-heroicon-s-arrow-right class="h-4" />
                                </a>
                            </p>
                            @if ($currentEntry->content)
                                <details>
                                    <summary>Read Content</summary>
                                    {!! Purify::clean($currentEntry->content) !!}
                                </details>
                            @endif
                        </article>
                    @endif
            ...
            <aside
                ...
                <ul class="absolute inset-0 divide-y divide-gray-200 dark:divide-black">
                    ...
                    @empty
                        <li class="px-6 py-6">No Unread articles in this feed</li>
                    @endforelse
                </ul>
